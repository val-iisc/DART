...
def interpolate_algos(sd1, sd2, sd3, sd4):
    return {key: (sd1[key] + sd2[key] + sd3[key] +sd4[key])/4 for key in sd1.keys()}
...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    batch_sizes50 = np.full([n_envs], int(hparams["batch_size"]*3*0.5), dtype=np.int)
    batch_sizes25 = np.full([n_envs], int(hparams["batch_size"]*3*0.25), dtype=np.int)
    ...
    # algorithmCE1.to(device)
    # algorithmCE2.to(device)
    # algorithmCE3.to(device)
    # algorithmCE4.to(device)
    algorithmCE1.cuda()
    algorithmCE2.cuda()
    algorithmCE3.cuda()
    algorithmCE4.cuda()
    ...
    train_minibatches_iterator25a = zip(*train_loaders25a)
    ...
    for step in range(n_steps):
        ...
        batches_dictlist25a = next(train_minibatches_iterator25a)
        batches_dictlist25b = next(train_minibatches_iterator25b)
        
        # batches: {x: [ <env1x>,<env2x>,<env3x> ] ,y: [ <env1y>,<env2y>,<env3y> ] }
        batchesCE = misc.merge_dictlist(batches_dictlist)
        batches1 = {'x': [ batches_dictlist50[0]['x'],batches_dictlist25a[1]['x'],batches_dictlist25b[2]['x'] ],
                    'y': [ batches_dictlist50[0]['y'],batches_dictlist25a[1]['y'],batches_dictlist25b[2]['y'] ] }
        batches2 = {'x': [ batches_dictlist25b[0]['x'],batches_dictlist50[1]['x'],batches_dictlist25a[2]['x'] ],
                    'y': [ batches_dictlist25b[0]['y'],batches_dictlist50[1]['y'],batches_dictlist25a[2]['y'] ] }
        batches3 = {'x': [ batches_dictlist25a[0]['x'],batches_dictlist25b[1]['x'],batches_dictlist50[2]['x'] ],
                    'y': [ batches_dictlist25a[0]['y'],batches_dictlist25b[1]['y'],batches_dictlist50[2]['y'] ] }
        
        # to device
        batchesCE = {key: [tensor.cuda() for tensor in tensorlist] for key, tensorlist in batchesCE.items()}
        batches1 = {key: [tensor.cuda() for tensor in tensorlist] for key, tensorlist in batches1.items()}
        batches2 = {key: [tensor.cuda() for tensor in tensorlist] for key, tensorlist in batches2.items()}
        batches3 = {key: [tensor.cuda() for tensor in tensorlist] for key, tensorlist in batches3.items()}
        ...
        step_valsCE4 = algorithmCE4.update(**inputsCE)
        ...
        if step % checkpoint_freq == 0:
            ...
            results_keys = list(summaries1.keys()) + list(summaries2.keys()) + list(summaries3.keys()) + list(summaries4.keys()) + list(results.keys())
            # merge results
            ...
            if swad1:
                ...
                if (hparams["model"]=='clip_vit-b16') and (step % 1500 == 0):
        ...
        if step%args.inter_freq==0 and step!=0:
            if args.algorithm in ['DANN', 'CDANN']:
                inter_state_dict = interpolate_algos(algorithmCE1.featurizer.state_dict(), algorithmCE2.featurizer.state_dict(), algorithmCE3.featurizer.state_dict(), algorithmCE4.featurizer.state_dict())
                ...
                inter_state_dict2 = interpolate_algos(algorithmCE1.classifier.state_dict(), algorithmCE2.classifier.state_dict(), algorithmCE3.classifier.state_dict(), algorithmCE4.classifier.state_dict())
                algorithmCE1.classifier.load_state_dict(inter_state_dict2)
                algorithmCE2.classifier.load_state_dict(inter_state_dict2)
                algorithmCE3.classifier.load_state_dict(inter_state_dict2)
                algorithmCE4.classifier.load_state_dict(inter_state_dict2)
                inter_state_dict3 = interpolate_algos(algorithmCE1.discriminator.state_dict(), algorithmCE2.discriminator.state_dict(), algorithmCE3.discriminator.state_dict(), algorithmCE4.discriminator.state_dict())
                algorithmCE1.discriminator.load_state_dict(inter_state_dict3)
                algorithmCE2.discriminator.load_state_dict(inter_state_dict3)
                algorithmCE3.discriminator.load_state_dict(inter_state_dict3)
                algorithmCE4.discriminator.load_state_dict(inter_state_dict3)
                
            elif args.algorithm in ['SagNet']:
                inter_state_dict = interpolate_algos(algorithmCE1.network_f.state_dict(), algorithmCE2.network_f.state_dict(), algorithmCE3.network_f.state_dict(), algorithmCE4.network_f.state_dict())
                ...
                inter_state_dict2 = interpolate_algos(algorithmCE1.network_c.state_dict(), algorithmCE2.network_c.state_dict(), algorithmCE3.network_c.state_dict(), algorithmCE4.network_c.state_dict())
                algorithmCE1.network_c.load_state_dict(inter_state_dict2)
                algorithmCE2.network_c.load_state_dict(inter_state_dict2)
                algorithmCE3.network_c.load_state_dict(inter_state_dict2)
                algorithmCE4.network_c.load_state_dict(inter_state_dict2)
                inter_state_dict3 = interpolate_algos(algorithmCE1.network_s.state_dict(), algorithmCE2.network_s.state_dict(), algorithmCE3.network_s.state_dict(), algorithmCE4.network_s.state_dict())
                algorithmCE1.network_s.load_state_dict(inter_state_dict3)
                algorithmCE2.network_s.load_state_dict(inter_state_dict3)
                algorithmCE3.network_s.load_state_dict(inter_state_dict3)
                algorithmCE4.network_s.load_state_dict(inter_state_dict3)
                
            else:
                inter_state_dict = interpolate_algos(algorithmCE1.network.state_dict(), algorithmCE2.network.state_dict(), algorithmCE3.network.state_dict(), algorithmCE4.network.state_dict())
    ...
    if swad1:
        ...
        swad_results = {**summaries_swad1, **summaries_swad2, **summaries_swad3, **summaries_swad4}
        step_str = f" [{swad_algorithm1.start_step}-{swad_algorithm1.end_step}]  (N={swad_algorithm1.n_averaged})  ||  [{swad_algorithm2.start_step}-{swad_algorithm2.end_step}]  (N={swad_algorithm2.n_averaged}) ||  [{swad_algorithm3.start_step}-{swad_algorithm3.end_step}]  (N={swad_algorithm3.n_averaged})  ||  [{swad_algorithm4.start_step}-{swad_algorithm4.end_step}]  (N={swad_algorithm4.n_averaged})"
        ...
        save_dict = {
            ...
            "model_hparams": dict(hparams),
            "test_envs": test_envs,
            "SWAD_1": swad_algorithm1.state_dict(),
            "SWAD_2": swad_algorithm2.state_dict(),
            "SWAD_3": swad_algorithm3.state_dict(),
            "SWAD_4": swad_algorithm4.state_dict(),
        }
        
        if args.algorithm in ['DANN', 'CDANN']:
            inter_state_dict = interpolate_algos(swad_algorithm1.module.featurizer.state_dict(), swad_algorithm2.module.featurizer.state_dict(), swad_algorithm3.module.featurizer.state_dict(), swad_algorithm4.module.featurizer.state_dict())
            swad_algorithm1.module.featurizer.load_state_dict(inter_state_dict)
            inter_state_dict2 = interpolate_algos(swad_algorithm1.module.classifier.state_dict(), swad_algorithm2.module.classifier.state_dict(), swad_algorithm3.module.classifier.state_dict(), swad_algorithm4.module.classifier.state_dict())
            swad_algorithm1.module.classifier.load_state_dict(inter_state_dict2)
            inter_state_dict3 = interpolate_algos(swad_algorithm1.module.discriminator.state_dict(), swad_algorithm2.module.discriminator.state_dict(), swad_algorithm3.module.discriminator.state_dict(), swad_algorithm4.module.discriminator.state_dict())
            swad_algorithm1.module.discriminator.load_state_dict(inter_state_dict3)
            
        elif args.algorithm in ['SagNet']:
            inter_state_dict = interpolate_algos(swad_algorithm1.module.network_f.state_dict(), swad_algorithm2.module.network_f.state_dict(), swad_algorithm3.module.network_f.state_dict(), swad_algorithm4.module.network_f.state_dict())
            swad_algorithm1.module.network_f.load_state_dict(inter_state_dict)
            inter_state_dict2 = interpolate_algos(swad_algorithm1.module.network_c.state_dict(), swad_algorithm2.module.network_c.state_dict(), swad_algorithm3.module.network_c.state_dict(), swad_algorithm4.module.network_c.state_dict())
            swad_algorithm1.module.network_c.load_state_dict(inter_state_dict2)
            inter_state_dict3 = interpolate_algos(swad_algorithm1.module.network_s.state_dict(), swad_algorithm2.module.network_s.state_dict(), swad_algorithm3.module.network_s.state_dict(), swad_algorithm4.module.network_s.state_dict())
            swad_algorithm1.module.network_s.load_state_dict(inter_state_dict3)
            
        else:
            inter_state_dict = interpolate_algos(swad_algorithm1.network.state_dict(), swad_algorithm2.network.state_dict(), swad_algorithm3.network.state_dict(), swad_algorithm4.network.state_dict())
